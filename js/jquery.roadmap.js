;(function (factory) {
    ...
    $.fn.roadmap = function (events, opts) {
        ...
        var defaults = {
            ...
            orientation: 'auto',
            onBuild: $.noop,
            eventTemplate: '<div class="event">' + '####CONTENT###' + '</div>'
        };

        var settings = $.extend({}, defaults, opts);

        var buildEvent = function (event, key) {
            var html = '<li id="###ID###" data-audioid="###AUDIOID###" class="' + settings.rootClass + '__events__event" onClick="getButtonId(this)">' + settings.eventTemplate + '</li>';
            html = html.replace('####DATE###', event.date);
            html = html.replace('####CONTENT###', event.content);
            html = html.replace('###ID###', event.id);
            html = html.replace('###AUDIOID###', event.id);
